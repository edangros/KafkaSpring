...
/**
 * Almost Same Spring-Kafka's {@link Kafkatemplate} and {@link ReplyingKafkaTemplate} but uses {@link SourceTask}s instead of {@link Producer}s.
 * 
 */
@Slf4j
public class ReplyingKafkaConnectTemplate implements BatchMessageListener<byte[], byte[]>, InitializingBean, SmartLifecycle,
        ApplicationContextAware, ApplicationListener<ContextStoppedEvent>, DisposableBean{
    ...
    public SettableListenableFuture<SinkRecord> sendAndReceive(SourceRecord record, Duration replyTimeout) {
        ...
        record.headers().add(KafkaHeaders.CORRELATION_ID,
                            Base64Utils.encode(correlationId.getCorrelationId()),
        ...
        RESTInputSourceTask task = RESTContextRegistry.getInstance().taskLoadBalancer(connectionID).getAppropriate();
    ...
    public SinkRecord convertRecord(ConsumerRecord<byte[],byte[]> record){
        ...
        try{
            ...
            //read kafka header and conver to connect header
    ...
    public void onApplicationEvent(ContextStoppedEvent event) {
		//Do nothing - We have No producer while Kafkatemplate has.
        //Tasks will stop by themselves.
    }
